...
produced 34.4 tok/s. The selected Huihui abliterated service sustains 167.2
tok/s on the full-context/cache-enabled 200 W representative fixture, **4.9x
the one-card control**. On the fixed p565/g512 power cell it reaches 142.5 tok/s
at 225 W; the stock target reached 180.3 tok/s on that workload. This
workload-dependent speculation delta is documented rather than hidden.
...
The live service exposes `qwen3.8-27b-abliterated` on port `19622` on every
host IPv4 interface. Local clients use `http://127.0.0.1:19622/v1`; LAN clients
use `http://<host-address>:19622/v1`. The endpoint is intentionally
unauthenticated and must remain restricted to trusted networks.

| Selected setting | Value |
|---|---|
| Physical GPUs | Two RTX 3090, PCIe 3.0 x16 |
| Tensor parallelism | 2 |
| Target | `Huihui-Qwen3.8-27B-Abliterated-W4A16-AutoRound` |
| Speculator | DFlash2 W4A16, 7 draft tokens |
| Context | 262,144 server; 253,952 input + 8,192 output for OpenCode |
| KV cache | BF16, 366,072 reported tokens on the final launch |
...
| Measurement | Result |
|---|---:|
| Huihui single stream, production profile at 200 W, approximately p512/g512 | **167.2 tok/s** |
| Huihui, two distinct 4K streams at 200 W | **269.7 decode aggregate tok/s** |
| Stock reference, four distinct 4K streams at 200 W | **309.8 decode aggregate tok/s** |
| Huihui DFlash2 power cell at 225 W, p565/g512 | **142.5 tok/s** |
| Huihui native MTP4, same power cell | **137.5 tok/s** |
| Stock DFlash2 reference, same power cell | **180.3 tok/s** |
| Stock cold-prefill power cell at 225 W, p8,221/g8 | **1,570.6 tok/s** |
| Stock cold 100,040-token retrieval | **86.67 s** |
| Stock repeated 100K retrieval | **1.07 s**, 99,456 cached tokens |
| Huihui OpenCode-style tool follow-up | **0.63 s**, 15,680 cached tokens |
| Stock real `opencode-cache` continuation | **17,920 cache-read tokens**, exact `CACHE_OK` |
...
differ. They converged on the same useful region for the stock target. Values
are medians and all output hashes remained identical across caps. The final
Huihui target still needs the planned separate decode/prefill efficiency sweep;
225 W/card remains the approved interim cap.
...
The stock-target power plateau is the cleanest measured hardware reference for
this machine. On the fixed decode cell, 225 W sustains 180.3 tok/s against
189.0 tok/s at 350–400 W:
...
| Separate stock-target prefill/decode power sweeps | Selected interim 225 W latency knee | Same prompt, output and hash at every cap |
| CPU-offloaded vision tower | Full 366,072-token final KV pool retained | Exact synthetic image canary |
| Stock to Huihui abliterated target | 163.2 → 167.2 tok/s at 200 W | 12/12 API, tool/cache and bounded quality gates rerun |
| Huihui speculator check on fixed 225 W cell | MTP4 137.5 → DFlash2 142.5 tok/s | DFlash2 retained; stock was 180.3 on this workload |
...
0.85 GiB tower CPU-offloaded, preserved the full KV pool and produced
...
image. The final launch reports 366,072 tokens of KV capacity with vision enabled.
...
1. Install a minimal vLLM systemd unit after the interactive profile is
   ...
   reboot.
2. Sweep the selected Huihui target for decode and prefill tokens/J separately;
   confirm the 225 W knee and then persist the chosen caps.
3. Run a real long OpenCode session through cache-preserving compaction and
   confirm cached-token accounting after compaction.
4. Convert the host to headless operation after client acceptance. GNOME/GDM
...
vllm serve /home/ai/qwen-serving/models/Huihui-Qwen3.8-27B-Abliterated-W4A16-AutoRound \
...
| `--gpu-memory-utilization 0.93` | Produces an 11.85 GiB KV allocation per worker and 366,072 reported tokens on the final launch. |
...
| `--served-model-name qwen3.8-27b-abliterated` | Makes the abliterated target explicit to clients. |
...
The selected Huihui W4A16 revision is
`c20530baefe3e77ccfc6891c2b50cce7ea28bf1e`; the stock comparison revision is
`1f05c441c4e64ae0549de44fa9ea5a6d43610314`, and the DFlash2 W4A16 revision is
...
Huihui is the selected target. It was 2.5% faster on the matched representative
single-stream fixture, but 21.0% slower than stock on the fixed p565/g512 power
prompt because speculative acceptance and generated token paths are
workload-dependent. Native MTP4 reached only 137.5 tok/s there, so DFlash2
remains selected at 142.5 tok/s. Aggregate perplexity was 0.29% worse and GSM8K
lost one item out of 200; the latter is well inside sampling noise. Other
available abliterated Qwen3.8-27B checkpoints either publish larger capability losses, lack a
comparable Ampere W4A16/MTP build, or both. Stock remains the rollback profile.
